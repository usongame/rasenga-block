@echo off
chcp 65001 >nul
title OpenBlock Driver Installer for Windows (x64)
echo ==========================================
echo OpenBlock Driver Installer for Windows (x64)
echo ==========================================
echo.
...
) else if exist "%DRIVER_PATH%\CH341SER64\SETUP.EXE" (
    echo Installing CH340 driver (x64)...
    start /wait "" "%DRIVER_PATH%\CH341SER64\SETUP.EXE"
    echo CH340 driver installation completed.
) else (
...
if exist "%DRIVER_PATH%\CP210x_Windows_Drivers\x64\silabser.sys" (
    echo Installing CP2102 driver...
    pnputil /add-driver "%DRIVER_PATH%\CP210x_Windows_Drivers\x64\silabser.inf" /install
    if %errorLevel% equ 0 (
...
if exist "%DRIVER_PATH%\ArduinoDrivers\dpinst-amd64.exe" (
    echo Installing Arduino drivers...
    start /wait "" "%DRIVER_PATH%\ArduinoDrivers\dpinst-amd64.exe" /sw /f
